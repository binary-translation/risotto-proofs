...
data SrcPo (x y : Event LabelTCG) : Set where
...
poʳ∈src : ∀ {x y : Event LabelTCG} → po src x y → y ∈ events src
...
po[⇒] : {x y : Event LabelTCG} → ¬ (x ≡ ev₂ × y ≡ ev₁) → po src x y → po dst x y
po[⇒] ¬xy (po-dst ¬[1x2y] po[xy]ᵗ) = po[xy]ᵗ
po[⇒] ¬xy (po-new x≡2 y≡1)         = ⊥-elim (¬xy (x≡2 , y≡1))

po[⇒]ˡ : {x y : Event LabelTCG} → x ≢ ev₂ → po src x y → po dst x y
po[⇒]ˡ = po[⇒] ∘ (_∘ proj₁)

po[⇒]ʳ : {x y : Event LabelTCG} → y ≢ ev₁ → po src x y → po dst x y
po[⇒]ʳ = po[⇒] ∘ (_∘ proj₂)

poˡ-1[⇒]2 : {y : Event LabelTCG} → po src ev₁ y → po dst ev₂ y
...
poʳ-2[⇒]1 : {x : Event LabelTCG} → po src x ev₂ → po dst x ev₁
...
po[⇐] : {x y : Event LabelTCG} → ¬ (x ≡ ev₁ × y ≡ ev₂) → po dst x y → po src x y
po[⇐] = po-dst

po[⇐]ˡ : {x y : Event LabelTCG} → x ≢ ev₁ → po dst x y → po src x y
po[⇐]ˡ = po[⇐] ∘ (_∘ proj₁)

po[⇐]ʳ : {x y : Event LabelTCG} → y ≢ ev₂ → po dst x y → po src x y
po[⇐]ʳ = po[⇐] ∘ (_∘ proj₂)

poʳ-1[⇐]2 : {x : Event LabelTCG} → po dst x ev₁ → po src x ev₂
poʳ-1[⇐]2 po[x1] =
  let x≢ev₁ = λ{refl → po-irreflexive dst-wf refl po[x1]}
  in po[⇐]ˡ x≢ev₁ (po-trans dst-wf po[x1] po[12]ᵗ)

poˡ-2[⇐]1 : {y : Event LabelTCG} → po dst ev₂ y → po src ev₁ y
poˡ-2[⇐]1 po[2y] =
...
piʳ-1[⇐]2 : {x : Event LabelTCG} → po-imm dst x ev₁ → po-imm src x ev₂
...
piˡ-2[⇐]1 : {y : Event LabelTCG} → po-imm dst ev₂ y → po-imm src ev₁ y
...
pi⁺ˡ-2[⇐]1 ( pi[2x] ∷ pi⁺[xy] ) =
  ...
  where
  pi[⇐]acc : {x y : Event LabelTCG} → po dst ev₂ x → po-imm dst x y → po-imm src x y
  pi[⇐]acc po[2x] pi[xy] =
...
udr-po[⇒] : {x : Event LabelTCG} → x ∈ udr (po src) → x ∈ udr (po dst)
...
udr-po[⇐] : {x : Event LabelTCG} → x ∈ udr (po dst) → x ∈ udr (po src)
...
private
  ...
  rw-split-r-rmw : ∀ {x : Event LabelTCG} → EvRW x → (EvRₜ trmw ∪₁ (EvRₜ tmov ∪₁ EvW)) x
...
ord[⇒] : {x y : Event LabelTCG} → OrdAlt src x y → OrdAlt dst x y
...
ord[⇒] (ord-rr ((refl , x-r) ⨾[ _ ]⨾ po[xz]ˢ ⨾[ _ ]⨾ (refl , z-rr) ⨾[ _ ]⨾ po[zy]ˢ ⨾[ _ ]⨾ (refl , y-r))) =
  let po[xz]ᵗ = po[⇒]rule pord-rrˡ x-r z-rr po[xz]ˢ
      po[zy]ᵗ = po[⇒]rule pord-rrʳ z-rr y-r po[zy]ˢ
  in ord-rr ((refl , x-r) ⨾[ _ ]⨾ po[xz]ᵗ ⨾[ _ ]⨾ (refl , z-rr) ⨾[ _ ]⨾ po[zy]ᵗ ⨾[ _ ]⨾ (refl , y-r))
ord[⇒] (ord-rw ((refl , x-r) ⨾[ _ ]⨾ po[xz]ˢ ⨾[ _ ]⨾ (refl , z-rw) ⨾[ _ ]⨾ po[zy]ˢ ⨾[ _ ]⨾ (refl , y-w))) =
  let po[xz]ᵗ = po[⇒]rule pord-rwˡ x-r z-rw po[xz]ˢ
      po[zy]ᵗ = po[⇒]rule pord-rwʳ z-rw y-w po[zy]ˢ
  in ord-rw ((refl , x-r) ⨾[ _ ]⨾ po[xz]ᵗ ⨾[ _ ]⨾ (refl , z-rw) ⨾[ _ ]⨾ po[zy]ᵗ ⨾[ _ ]⨾ (refl , y-w))
ord[⇒] (ord-rm ((refl , x-r) ⨾[ _ ]⨾ po[xz]ˢ ⨾[ _ ]⨾ (refl , z-rm) ⨾[ _ ]⨾ po[zy]ˢ ⨾[ _ ]⨾ (refl , y-rw))) =
  let po[xz]ᵗ = po[⇒]rule pord-rmˡ x-r z-rm po[xz]ˢ
      po[zy]ᵗ = po[⇒]rule pord-rmʳ z-rm y-rw po[zy]ˢ
  in ord-rm ((refl , x-r) ⨾[ _ ]⨾ po[xz]ᵗ ⨾[ _ ]⨾ (refl , z-rm) ⨾[ _ ]⨾ po[zy]ᵗ ⨾[ _ ]⨾ (refl , y-rw))

ord[⇒] (ord-wr ((refl , x-w) ⨾[ _ ]⨾ po[xz]ˢ ⨾[ _ ]⨾ (refl , z-rr) ⨾[ _ ]⨾ po[zy]ˢ ⨾[ _ ]⨾ (refl , y-r))) =
  let po[xz]ᵗ = po[⇒]rule pord-wrˡ x-w z-rr po[xz]ˢ
      po[zy]ᵗ = po[⇒]rule pord-wrʳ z-rr y-r po[zy]ˢ
  in ord-wr ((refl , x-w) ⨾[ _ ]⨾ po[xz]ᵗ ⨾[ _ ]⨾ (refl , z-rr) ⨾[ _ ]⨾ po[zy]ᵗ ⨾[ _ ]⨾ (refl , y-r))
ord[⇒] (ord-ww ((refl , x-w) ⨾[ _ ]⨾ po[xz]ˢ ⨾[ _ ]⨾ (refl , z-rw) ⨾[ _ ]⨾ po[zy]ˢ ⨾[ _ ]⨾ (refl , y-w))) =
  let po[xz]ᵗ = po[⇒]rule pord-wwˡ x-w z-rw po[xz]ˢ
      po[zy]ᵗ = po[⇒]rule pord-wwʳ z-rw y-w po[zy]ˢ
  in ord-ww ((refl , x-w) ⨾[ _ ]⨾ po[xz]ᵗ ⨾[ _ ]⨾ (refl , z-rw) ⨾[ _ ]⨾ po[zy]ᵗ ⨾[ _ ]⨾ (refl , y-w))
ord[⇒] (ord-wm ((refl , x-w) ⨾[ _ ]⨾ po[xz]ˢ ⨾[ _ ]⨾ (refl , z-rm) ⨾[ _ ]⨾ po[zy]ˢ ⨾[ _ ]⨾ (refl , y-rw))) =
  let po[xz]ᵗ = po[⇒]rule pord-wmˡ x-w z-rm po[xz]ˢ
      po[zy]ᵗ = po[⇒]rule pord-wmʳ z-rm y-rw po[zy]ˢ
  in ord-wm ((refl , x-w) ⨾[ _ ]⨾ po[xz]ᵗ ⨾[ _ ]⨾ (refl , z-rm) ⨾[ _ ]⨾ po[zy]ᵗ ⨾[ _ ]⨾ (refl , y-rw))

ord[⇒] (ord-mr ((refl , x-rw) ⨾[ _ ]⨾ po[xz]ˢ ⨾[ _ ]⨾ (refl , z-rr) ⨾[ _ ]⨾ po[zy]ˢ ⨾[ _ ]⨾ (refl , y-r))) =
  let po[xz]ᵗ = po[⇒]rule pord-mrˡ x-rw z-rr po[xz]ˢ
      po[zy]ᵗ = po[⇒]rule pord-mrʳ z-rr y-r po[zy]ˢ
  in ord-mr ((refl , x-rw) ⨾[ _ ]⨾ po[xz]ᵗ ⨾[ _ ]⨾ (refl , z-rr) ⨾[ _ ]⨾ po[zy]ᵗ ⨾[ _ ]⨾ (refl , y-r))
ord[⇒] (ord-mw ((refl , x-rw) ⨾[ _ ]⨾ po[xz]ˢ ⨾[ _ ]⨾ (refl , z-rw) ⨾[ _ ]⨾ po[zy]ˢ ⨾[ _ ]⨾ (refl , y-w))) =
  let po[xz]ᵗ = po[⇒]rule pord-mwˡ x-rw z-rw po[xz]ˢ
      po[zy]ᵗ = po[⇒]rule pord-mwʳ z-rw y-w po[zy]ˢ
  in ord-mw ((refl , x-rw) ⨾[ _ ]⨾ po[xz]ᵗ ⨾[ _ ]⨾ (refl , z-rw) ⨾[ _ ]⨾ po[zy]ᵗ ⨾[ _ ]⨾ (refl , y-w))
ord[⇒] (ord-mm ((refl , x-rw) ⨾[ _ ]⨾ po[xz]ˢ ⨾[ _ ]⨾ (refl , z-rm) ⨾[ _ ]⨾ po[zy]ˢ ⨾[ _ ]⨾ (refl , y-rw))) =
  let po[xz]ᵗ = po[⇒]rule pord-mmˡ x-rw z-rm po[xz]ˢ
      po[zy]ᵗ = po[⇒]rule pord-mmʳ z-rm y-rw po[zy]ˢ
  in ord-mm ((refl , x-rw) ⨾[ _ ]⨾ po[xz]ᵗ ⨾[ _ ]⨾ (refl , z-rm) ⨾[ _ ]⨾ po[zy]ᵗ ⨾[ _ ]⨾ (refl , y-rw))

ord[⇒] (ord-sc ((refl , x-rw) ⨾[ x ]⨾ po[xz]ˢ ⨾[ z ]⨾ (refl , z-sc) ⨾[ z ]⨾ po[zy]ˢ ⨾[ y ]⨾ (refl , y-rw))) =
  let po[xz]ᵗ = po[⇒]rule pord-scˡ x-rw z-sc po[xz]ˢ
