...
src-rmw-def = ⊆: lemma
  ...
  lemma x y rmw[xy]ˢ =
    let rmw[xy]ᵗ = rmw[xy]ˢ
        ...
        x-rₐ = rmwˡ-r dst-wf (_ , rmw[xy]ˢ)
        y-wₐ = rmwʳ-w dst-wf (_ , rmw[xy]ˢ)
        
        ¬xy : ¬ (x ≡ ev₁ × y ≡ ev₂)
        ¬xy = λ{(x≡1 , _) → ¬ra₁ (subst (EvRₜ trmw) x≡1 x-rₐ)}
        
        y≢ev₁ = λ{y≡1 → ¬wa₁ (subst (EvWₜ trmw) y≡1 y-wₐ)}
        x≢ev₂ = λ{x≡2 → ¬ra₂ (subst (EvRₜ trmw) x≡2 x-rₐ)}
