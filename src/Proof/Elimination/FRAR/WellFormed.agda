...
module Proof.Elimination.FRAR.WellFormed
...
src-rf-sloc = ⊆: lemma
  ...
  lemma _ _ (rf-new dst-rf[xp] refl refl) =
    ...
    in trans-same-loc
         (sloc[⇐$] (events[⇐] x∈dst) (events[⇐] (pres₁∈ex dst-ok)) xp-sloc)
         (subst-rel SameLoc src-pres₁-def refl pe-sloc)
...
src-rf-sval = ⊆: lemma
  ...
  lemma _ _ (rf-new dst-rf[xp] refl refl) =
    ...
    in trans-same-val (sval[⇐$] (events[⇐] x∈dst) (events[⇐] (pres₁∈ex dst-ok)) xy-sval) (subst-rel SameVal src-pres₁-def refl pe-sval)
...
src-wf-rf-≥1 = ⊆: lemma
  ...
  ... | yes refl =
    let (y , rf[yp]) = ⊆₁-apply (wf-rf-≥1 dst-wf) (pres₁-r dst-ok , pres₁∈ex dst-ok)
...
... | yes refl =
  let y₁∈src = rfˡ∈src rf[y₁x]
      ...
      ¬elim-y₂ = elim-¬w (×₂-applyˡ src-rf-w×r rf[y₂x])
      dst-rf[y₁p] = rf[⇒] ¬elim-y₁ ¬pres₁-elimₛ y₁∈src pres₁∈src (rfʳ-e⇒p rf[y₁x])
      dst-rf[y₂p] = rf[⇒] ¬elim-y₂ ¬pres₁-elimₛ y₂∈src pres₁∈src (rfʳ-e⇒p rf[y₂x])
