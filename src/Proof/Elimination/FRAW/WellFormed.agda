...
module Proof.Elimination.FRAW.WellFormed
...
src-rf-sloc = ⊆: lemma
  ...
  lemma _ _ (rf-new refl refl) = p₁e-sloc
...
src-wf-rf-≥1 = ⊆: lemma
  ...
  ... | yes refl = src-pres₁-ev , (rf-new refl refl)
  ... | no ¬x-elim =
...
... | yes refl = -- x ≡ e
  ...
  where
  rf[xe]⇒x≡p : {x : Event LabelTCG} → rf src x src-elim-ev → x ≡ src-pres₁-ev
  rf[xe]⇒x≡p (rf-dst (x , y , rf[xy] , refl , p))
    with ev[$⇒]eq (elim∈ex dst-ok) (rfʳ∈ex dst-wf rf[xy]) p -- e ≡ y in target?
  ... | refl = ⊥-elim (disjoint-r/skip _ (×₂-applyʳ (rf-w×r dst-wf) rf[xy] , elim-skip dst-ok))
