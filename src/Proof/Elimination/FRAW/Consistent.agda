...
--
-- The Read-after-Write elimination /is only correct/ on TCG programs
-- lifted from x86.
data OrdFRAW (x y : Event LabelTCG) : Set where
  ord-init      : ( ⦗ EvInit ⦘ ⨾ po src ⨾ ⦗ EvRW ⦘ )                      x y → OrdFRAW x y
  ord-ww        : ( ⦗ EvW ⦘  ⨾ po src ⨾ ⦗ EvF₌ WW ⦘ ⨾ po src ⨾ ⦗ EvW ⦘ )  x y → OrdFRAW x y
  ord-rm        : ( ⦗ EvR ⦘  ⨾ po src ⨾ ⦗ EvF₌ RM ⦘ ⨾ po src ⨾ ⦗ EvRW ⦘ ) x y → OrdFRAW x y
  ord-sc        : ( ⦗ EvRW ⦘ ⨾ po src ⨾ ⦗ EvF₌ SC ⦘ ⨾ po src ⨾ ⦗ EvRW ⦘ ) x y → OrdFRAW x y
  ord-rmw-dom   : ( ⦗ EvRW ⦘ ⨾ po src ⨾ ⦗ dom (rmw src) ⦘ )               x y → OrdFRAW x y
  ord-rmw-codom : ( ⦗ codom (rmw src) ⦘ ⨾ po src ⨾ ⦗ EvRW ⦘ )             x y → OrdFRAW x y
  ord-w         : ( ⦗ EvRW ⦘ ⨾ po src ⨾ ⦗ EvWₜ trmw ⦘ )                   x y → OrdFRAW x y
  ord-r         : ( ⦗ EvRₜ trmw ⦘ ⨾ po src ⨾ ⦗ EvRW ⦘ )                   x y → OrdFRAW x y
...
... | ev-f₌ = absurd (⊆₁-apply events-bound (events[⇒] x∈src)) λ()
...
OrdAlt⇒OrdFRAW : ∀ {x y : Event LabelTCG} → OrdAlt src x y → OrdFRAW x y
OrdAlt⇒OrdFRAW (ord-init init[xy])    = ord-init init[xy]
OrdAlt⇒OrdFRAW (ord-rm rm[xy])        = ord-rm rm[xy]
OrdAlt⇒OrdFRAW (ord-ww ww[xy])        = ord-ww ww[xy]
OrdAlt⇒OrdFRAW (ord-sc sc[xy])        = ord-sc sc[xy]
OrdAlt⇒OrdFRAW (ord-rmw-dom po[xy])   = ord-rmw-dom po[xy]
OrdAlt⇒OrdFRAW (ord-rmw-codom po[xy]) = ord-rmw-codom po[xy]
OrdAlt⇒OrdFRAW (ord-w po[xy])         = ord-w po[xy]
OrdAlt⇒OrdFRAW (ord-r po[xy])         = ord-r po[xy]
-- impossible
OrdAlt⇒OrdFRAW (ord-rr rr[xy]) = ⊥-elim (src-no-rr _ (proj₂ (ord-fence∈src rr[xy])))
OrdAlt⇒OrdFRAW (ord-rw rw[xy]) = ⊥-elim (src-no-rw _ (proj₂ (ord-fence∈src rw[xy])))
OrdAlt⇒OrdFRAW (ord-wr wr[xy]) = ⊥-elim (src-no-wr _ (proj₂ (ord-fence∈src wr[xy])))
OrdAlt⇒OrdFRAW (ord-wm wm[xy]) = ⊥-elim (src-no-wm _ (proj₂ (ord-fence∈src wm[xy])))
OrdAlt⇒OrdFRAW (ord-mr mr[xy]) = ⊥-elim (src-no-mr _ (proj₂ (ord-fence∈src mr[xy])))
OrdAlt⇒OrdFRAW (ord-mw mw[xy]) = ⊥-elim (src-no-mw _ (proj₂ (ord-fence∈src mw[xy])))
OrdAlt⇒OrdFRAW (ord-mm mm[xy]) = ⊥-elim (src-no-mm _ (proj₂ (ord-fence∈src mm[xy])))


OrdFRAW⇒OrdAlt : ∀ {x y : Event LabelTCG} → OrdFRAW x y → OrdAlt src x y
OrdFRAW⇒OrdAlt (ord-init init[xy])    = ord-init init[xy]
OrdFRAW⇒OrdAlt (ord-ww ww[xy])        = ord-ww ww[xy]
OrdFRAW⇒OrdAlt (ord-rm rm[xy])        = ord-rm rm[xy]
OrdFRAW⇒OrdAlt (ord-sc sc[xy])        = ord-sc sc[xy]
OrdFRAW⇒OrdAlt (ord-rmw-dom po[xy])   = ord-rmw-dom po[xy]
OrdFRAW⇒OrdAlt (ord-rmw-codom po[xy]) = ord-rmw-codom po[xy]
OrdFRAW⇒OrdAlt (ord-w po[xy])         = ord-w po[xy]
OrdFRAW⇒OrdAlt (ord-r po[xy])         = ord-r po[xy]
...
src-p₁e-sloc = same-loc (pres₁-has-loc dst-ok) src-elim-has-loc

poˡ-e⇒p : {y : Event LabelTCG} → po src src-elim-ev y → po src src-pres₁-ev y
poˡ-e⇒p = po-trans src-wf po[p₁e]ˢ

plˡ-e⇒p : {y : Event LabelTCG} → po-loc src src-elim-ev y → po-loc src src-pres₁-ev y
plˡ-e⇒p (po[ey] , ey-sloc) = poˡ-e⇒p po[ey] , trans-same-loc src-p₁e-sloc ey-sloc

poʳ-e⇒p₁ : {x : Event LabelTCG} → x ≢ src-pres₁-ev → x ≢ src-pres₂-ev → po src x src-elim-ev → po src x src-pres₁-ev
...
plʳ-e⇒p₁ : {x : Event LabelTCG} → x ≢ src-pres₁-ev → po-loc src x src-elim-ev → po-loc src x src-pres₁-ev
...
poˡ-p₁⇒e : {y : Event LabelTCG} → y ≢ src-pres₂-ev → y ≢ src-elim-ev → po src src-pres₁-ev y → po src src-elim-ev y
...
¬poˡ-e⇒p₁ : {y : Event LabelTCG} → y ≢ src-pres₂-ev → y ≢ src-elim-ev → ¬ po src src-elim-ev y → ¬ po src src-pres₁-ev y
¬poˡ-e⇒p₁ = contrapositive ∘₂ poˡ-p₁⇒e

fr[ey]→co[py] : {y : Event LabelTCG} → fr src src-elim-ev y → co src src-pres₁-ev y
fr[ey]→co[py] (rf⁻¹[ez] ⨾[ _ ]⨾ co[zy]) with wf-rf-≤1 src-wf _ _ _ rf-p₁e rf⁻¹[ez]
... | refl = co[zy]


ordˡ-e⇒p : {y : Event LabelTCG} → OrdFRAW src-elim-ev y → OrdFRAW src-pres₁-ev y
ordˡ-e⇒p (ord-rm ((refl , _) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-rm) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-rw))) =
  ...
  in ord-sc ((refl , w⇒rw (pres₁-w dst-ok)) ⨾[ _ ]⨾ proj₁ pi[p₁p₂]ˢ ⨾[ _ ]⨾ (refl , pres₂-f₌ dst-ok) ⨾[ _ ]⨾ po[p₂y] ⨾[ _ ]⨾ (refl , y-rw))
...
ordˡ-e⇒p (ord-rmw-dom ((refl , e-rw) ⨾[ _ ]⨾ po[ey] ⨾[ _ ]⨾ (refl , y∈rmwˡ))) =
  ...
  in ord-rmw-dom ((refl , w⇒rw (pres₁-w dst-ok)) ⨾[ _ ]⨾ po[p₁y] ⨾[ _ ]⨾ (refl , y∈rmwˡ))
ordˡ-e⇒p (ord-w ((refl , e-rw) ⨾[ _ ]⨾ po[ey] ⨾[ _ ]⨾ (refl , y-wₜ))) =
  ord-w ((refl , w⇒rw (pres₁-w dst-ok)) ⨾[ _ ]⨾ poˡ-e⇒p po[ey] ⨾[ _ ]⨾ (refl , y-wₜ))
-- Impossible cases
...
src-po-r-rmʳ : {x : Event LabelTCG} → x ≢ src-elim-ev → x ∈ events src → EvRᵣ x → ∃[ y ] (EvF₌ RM y × po-imm src x y)
...
src-po-w-wwˡ : {y : Event LabelTCG} → y ≢ src-elim-ev → y ∈ events src → EvWₙₜ tmov y → ∃[ x ] (EvF₌ WW x × po-imm src x y)
...
ordWWʳ-e⇒p₁ : {x : Event LabelTCG}
  → x ≢ src-pres₁-ev
  → ( ⦗ EvRW ⦘ ⨾ po src ⨾ ⦗ EvF₌ SC ⦘ ⨾ po src ⨾ ⦗ EvRW ⦘ ) x src-elim-ev
  → OrdFRAW x src-pres₁-ev
ordWWʳ-e⇒p₁ {x} ¬x-pres₁ ((refl , x-rw) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-sc) ⨾[ z ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-rw)) =
  ...
  where
  lemma₁ : po src z src-pres₂-ev → OrdFRAW x src-pres₁-ev
  lemma₁ po[zp₂] =
    let ¬z-pres₁ = λ{z≡p₁ → disjoint-f/w _ (subst EvF z≡p₁ (f₌⇒f z-sc) , pres₁-w dst-ok)}
        po[zp₁] = elim-⊥⊎ ¬z-pres₁ id (unsplit-poʳ src-wf po[zp₂] pi[p₁p₂]ˢ)
    in ord-sc ((refl , x-rw) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-sc) ⨾[ _ ]⨾ po[zp₁] ⨾[ _ ]⨾ (refl , w⇒rw (pres₁-w dst-ok)))

  lemma₂ : po src x src-pres₂-ev → OrdFRAW x src-pres₁-ev
  lemma₂ po[xp₂] =
    ...
    λ{ (inj₁ x-r) →
         ...
         λ{ (inj₁ x-rᵣ) →
              let ¬x-init = λ{x-init → disjoint-r/init _ (x-r , x-init)}
                  ...
                  ¬y-pres₁ = λ{y≡p₁ → disjoint-f/w _ (subst EvF y≡p₁ (f₌⇒f y-rm) , pres₁-w dst-ok)}
                  po[yp₁] = elim-⊥⊎ ¬y-pres₁ id (unsplit-poˡ src-wf ¬x-init po[xp₁] pi[xy])
              in ord-rm ((refl , x-r) ⨾[ _ ]⨾ proj₁ pi[xy] ⨾[ _ ]⨾ (refl , y-rm) ⨾[ _ ]⨾ po[yp₁] ⨾[ _ ]⨾ (refl , w⇒rw (pres₁-w dst-ok)))
          ; (inj₂ x-rₐ) →
              ord-r ((refl , x-rₐ) ⨾[ _ ]⨾ po[xp₁] ⨾[ _ ]⨾ (refl , w⇒rw (pres₁-w dst-ok)))
          }
     ; (inj₂ x-w) →
         ...
         in ord-ww ((refl , x-w) ⨾[ _ ]⨾ po[xy] ⨾[ _ ]⨾ (refl , y-ww) ⨾[ _ ]⨾ proj₁ pi[yp₁] ⨾[ _ ]⨾ (refl , pres₁-w dst-ok))
     }


ordʳ-e⇒p : {x : Event LabelTCG} → x ≢ src-pres₁-ev → OrdFRAW x src-elim-ev → OrdFRAW x src-pres₁-ev
ordʳ-e⇒p ¬x-pres₁ (ord-init ((refl , x-init) ⨾[ _ ]⨾ po[xe] ⨾[ _ ]⨾ (refl , e-rw))) =
  ...
  in ord-init ((refl , x-init) ⨾[ _ ]⨾ po[xp₁] ⨾[ _ ]⨾ (refl , w⇒rw (pres₁-w dst-ok)))
ordʳ-e⇒p _ (ord-rm ((refl , x-r) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-rm) ⨾[ _ ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-rw))) =
  let ¬z-pres₁ = λ{z≡p₁ → disjoint-f/w _ (subst EvF z≡p₁ (f₌⇒f z-rm) , pres₁-w dst-ok)}
      ¬z-pres₂ = λ{z≡p₂ → disjoint-f/mode (λ()) _ (pres₂-f₌ dst-ok , subst (EvF₌ RM) z≡p₂ z-rm)}
      po[zp₁] = poʳ-e⇒p₁ ¬z-pres₁ ¬z-pres₂ po[ze]
  in ord-rm ((refl , x-r) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-rm) ⨾[ _ ]⨾ po[zp₁] ⨾[ _ ]⨾ (refl , w⇒rw (pres₁-w dst-ok)))
...
ordʳ-e⇒p ¬x-pres₁ (ord-rmw-codom ((refl , x∈rmwʳ) ⨾[ _ ]⨾ po[xe] ⨾[ _ ]⨾ (refl , e-rw))) =
  let ¬x-pres₂ = λ{x≡p₂ → disjoint-f/w _ (pres₂-f , subst EvW x≡p₂ (wₜ⇒w (rmwʳ-w src-wf x∈rmwʳ)))}
      po[xp₁] = poʳ-e⇒p₁ ¬x-pres₁ ¬x-pres₂ po[xe]
  in ord-rmw-codom ((refl , x∈rmwʳ) ⨾[ _ ]⨾ po[xp₁] ⨾[ _ ]⨾ (refl , w⇒rw (pres₁-w dst-ok)))
...
ordʳ-e⇒p ¬x-pres₁ (ord-r ((refl , x-rₐ) ⨾[ _ ]⨾ po[xe] ⨾[ _ ]⨾ (refl , e-rw))) =
  let ¬x-pres₂ = λ{x≡p₂ → disjoint-f/r _ (pres₂-f , subst EvR x≡p₂ (rₜ⇒r x-rₐ))}
      po[xp₁] = poʳ-e⇒p₁ ¬x-pres₁ ¬x-pres₂ po[xe]
  in ord-r ((refl , x-rₐ) ⨾[ _ ]⨾ po[xp₁] ⨾[ _ ]⨾ (refl , w⇒rw (pres₁-w dst-ok)))
-- Impossible cases
ordʳ-e⇒p ¬x-pres₁ (ord-ww ww[xe]) = ⊥-elim (disjoint-r/w _ (src-elim-r , ord-predʳ src ww[xe]))


frˡ-e⇒coˡ-p : {y : Event LabelTCG} → fr src src-elim-ev y → co src src-pres₁-ev y
frˡ-e⇒coˡ-p (rf⁻¹[ez] ⨾[ _ ]⨾ co[zy]) with wf-rf-≤1 src-wf _ _ _ rf⁻¹[ez] rf-p₁e
... | refl = co[zy]

freˡ-e⇒coeˡ-p : {y : Event LabelTCG} → fre src src-elim-ev y → coe src src-pres₁-ev y
...
ghbiˡ-e⇒p : {y : Event LabelTCG} → GhbiAlt src src-elim-ev y → GhbiAlt src src-pres₁-ev y
ghbiˡ-e⇒p (ghbi-ord ord[ey]) = ghbi-ord (OrdFRAW⇒OrdAlt (ordˡ-e⇒p (OrdAlt⇒OrdFRAW ord[ey])))
...
ghbiʳ-e⇒p : {x : Event LabelTCG} → x ≢ src-pres₁-ev → GhbiAlt src x src-elim-ev → GhbiAlt src x src-pres₁-ev
ghbiʳ-e⇒p ¬x-pres₁ (ghbi-ord ord[xe]) = ghbi-ord (OrdFRAW⇒OrdAlt (ordʳ-e⇒p ¬x-pres₁ (OrdAlt⇒OrdFRAW ord[xe])))
-- impossible cases
