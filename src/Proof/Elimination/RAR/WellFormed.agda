...
src-rf-sloc = ⊆: lemma
  ...
  lemma _ _ (rf-new dst-rf[xp] refl refl) =
    ...
    in trans-same-loc
         (sloc[⇐$] (events[⇐] x∈dst) (events[⇐] (preserved∈ex dst-ok)) xp-sloc)
...
src-rf-sval = ⊆: lemma
  ...
  lemma _ _ (rf-new dst-rf[xp] refl refl) =
    ...
    in trans-same-val (sval[⇐$] (events[⇐] x∈dst) (events[⇐] (preserved∈ex dst-ok)) xy-sval) (subst-rel SameVal src-preserved-def refl pe-sval)
...
src-wf-rf-≥1 = ⊆: lemma
  ...
  ... | yes refl =
    let (y , rf[yp]) = ⊆₁-apply (wf-rf-≥1 dst-wf) (rₜ⇒r (preserved-ev-r dst-ok) , preserved∈ex dst-ok)
