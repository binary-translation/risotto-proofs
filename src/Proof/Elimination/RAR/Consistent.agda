...
poˡ-p⇒e ¬elim-y po[py] = 
  ...
  in elim-⊥⊎ (λ{refl → ¬elim-y refl}) (λ{τ → τ}) lemma
  where
  pres-¬init : ¬ EvInit src-preserved-ev
  pres-¬init ev-init = disjoint-r/init _ (rₜ⇒r (preserved-ev-r dst-ok) , ev-init)

poʳ-p⇒e : {x : Event LabelTCG} → po src x src-preserved-ev → po src x src-elim-ev
...
¬poʳ-e⇒p : {x : Event LabelTCG} → ¬ po src x src-elim-ev → ¬ po src x src-preserved-ev
¬poʳ-e⇒p ¬po[xe] = ¬po[xe] ∘ poʳ-p⇒e


freˡ-e⇒p : {y : Event LabelTCG} → fre src src-elim-ev y → fre src src-preserved-ev y
freˡ-e⇒p (fr[ey] , ¬po[ey]) =
  let ¬y-elim = λ{refl → fr-irreflexive src-wf refl fr[ey]}
  in frˡ-e⇒p fr[ey] , ¬poˡ-e⇒p ¬y-elim ¬po[ey]

rfeʳ-e⇒p : {x : Event LabelTCG} → rfe src x src-elim-ev → rfe src x src-preserved-ev
rfeʳ-e⇒p (rf[xe] , ¬po[xe]) = rfʳ-e⇒p rf[xe] , ¬poʳ-e⇒p ¬po[xe]

pres-r : EvR src-preserved-ev
pres-r = rₜ⇒r (preserved-ev-r dst-ok)

pres-rw : EvRW src-preserved-ev
pres-rw = rₜ⇒rw (preserved-ev-r dst-ok)

¬init-pres : {x : Event LabelTCG} → EvInit x → x ≢ src-preserved-ev
¬init-pres ev-init refl = disjoint-r/init _ (pres-r , ev-init)

¬f-pres : {x : Event LabelTCG} → EvF x → x ≢ src-preserved-ev
¬f-pres x-f refl = disjoint-f/r _ (x-f , pres-r)

¬w-pres : {x : Event LabelTCG} → EvW x → x ≢ src-preserved-ev
...
cohˡ-e⇒p : {y : Event LabelTCG} → Coh src src-elim-ev y → Coh src src-preserved-ev y
...
cohʳ-e⇒p : {x : Event LabelTCG} → x ≢ src-preserved-ev → Coh src x src-elim-ev → Coh src x src-preserved-ev
...
frˡ∈src : {x y : Event LabelTCG} → fr src x y → x ∈ events src
...
ordʳ-e⇒p : ∀ {x : Event LabelTCG} → OrdAlt src x src-elim-ev → OrdAlt src x src-preserved-ev
...
-- RR
ordʳ-e⇒p (ord-rr ((refl , x-r) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-r))) =
  let po[zp] = poʳ-e⇒p (¬f-pres (f₌⇒f z-f)) po[ze]
  in ord-rr ((refl , x-r) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zp] ⨾[ _ ]⨾ (refl , pres-r))
-- RM
...
-- WR
ordʳ-e⇒p (ord-wr ((refl , x-w) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-r))) =
  let po[zp] = poʳ-e⇒p (¬f-pres (f₌⇒f z-f)) po[ze]
  in ord-wr ((refl , x-w) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zp] ⨾[ _ ]⨾ (refl , pres-r))
-- WM
ordʳ-e⇒p (ord-wm ((refl , x-w) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-m))) =
  let po[zp] = poʳ-e⇒p (¬f-pres (f₌⇒f z-f)) po[ze]
  in ord-wm ((refl , x-w) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zp] ⨾[ _ ]⨾ (refl , pres-rw))
-- MR
ordʳ-e⇒p (ord-mr ((refl , x-m) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-r))) =
  let po[zp] = poʳ-e⇒p (¬f-pres (f₌⇒f z-f)) po[ze]
  in ord-mr ((refl , x-m) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zp] ⨾[ _ ]⨾ (refl , pres-r))
-- MM
ordʳ-e⇒p (ord-mm ((refl , x-m) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-m))) =
  let po[zp] = poʳ-e⇒p (¬f-pres (f₌⇒f z-f)) po[ze]
  in ord-mm ((refl , x-m) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zp] ⨾[ _ ]⨾ (refl , pres-rw))
-- SC
...
ordʳ-e⇒p {x} (ord-r ((refl , x-rₜ) ⨾[ _ ]⨾ po[xe] ⨾[ _ ]⨾ (refl , e-rw))) =
  let ¬x-pres = λ{refl → disjoint-rₜ _ (preserved-ev-r dst-ok , x-rₜ)}
      po[xp] = poʳ-e⇒p ¬x-pres po[xe]
  in ord-r ((refl , x-rₜ) ⨾[ _ ]⨾ po[xp] ⨾[ _ ]⨾ (refl , pres-rw))
-- impossible cases
-- RW
ordʳ-e⇒p (ord-rw ((refl , x-r) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-w))) =
  ⊥-elim (disjoint-r/w _ (rₜ⇒r elim-rₜ , e-w))
-- WW
ordʳ-e⇒p (ord-ww ((refl , x-w) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-w))) =
  ⊥-elim (disjoint-r/w _ (rₜ⇒r elim-rₜ , e-w))
-- MW
ordʳ-e⇒p (ord-mw ((refl , x-m) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[ze] ⨾[ _ ]⨾ (refl , e-w))) =
  ⊥-elim (disjoint-r/w _ (rₜ⇒r elim-rₜ , e-w))
-- rmwˡ
...
ordˡ-e⇒p : ∀ {y : Event LabelTCG} → OrdAlt src src-elim-ev y → OrdAlt src src-preserved-ev y
ordˡ-e⇒p (ord-rr ((refl , e-r) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-r))) =
  ord-rr ((refl , pres-r) ⨾[ _ ]⨾ poˡ-e⇒p po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-r))
ordˡ-e⇒p (ord-rw ((refl , e-r) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-w))) =
  ord-rw ((refl , pres-r) ⨾[ _ ]⨾ poˡ-e⇒p po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-w))
...
ordˡ-e⇒p (ord-mr ((refl , e-m) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-r))) =
  ord-mr ((refl , pres-rw) ⨾[ _ ]⨾ poˡ-e⇒p po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-r))
ordˡ-e⇒p (ord-mw ((refl , e-m) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-w))) =
  ord-mw ((refl , pres-rw) ⨾[ _ ]⨾ poˡ-e⇒p po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-w))
ordˡ-e⇒p (ord-mm ((refl , e-m) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-m))) =
  ord-mm ((refl , pres-rw) ⨾[ _ ]⨾ poˡ-e⇒p po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-m))
...
ordˡ-e⇒p (ord-wr ((refl , e-w) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-r))) =
  ⊥-elim (disjoint-r/w _ (rₜ⇒r elim-rₜ , e-w))
ordˡ-e⇒p (ord-ww ((refl , e-w) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-w))) =
  ⊥-elim (disjoint-r/w _ (rₜ⇒r elim-rₜ , e-w))
ordˡ-e⇒p (ord-wm ((refl , e-w) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-m))) =
  ⊥-elim (disjoint-r/w _ (rₜ⇒r elim-rₜ , e-w))
...
ghbiˡ-e⇒p : {y : Event LabelTCG} → GhbiAlt src src-elim-ev y → GhbiAlt src src-preserved-ev y
...
ghbiʳ-e⇒p : {x : Event LabelTCG} → GhbiAlt src x src-elim-ev → GhbiAlt src x src-preserved-ev
