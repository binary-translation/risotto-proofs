...
import Proof.Framework LabelTCG dst (RAR-Restricted.wf dst-ok) as Ψ
...
data SrcRf (x y : Event LabelTCG) : Set where
  ...
  rf-new : {x-dst : Event LabelTCG}
      (dst-rf[xy] : rf dst x-dst (preserved-ev dst-ok))
...
module Extra where
  ...
  src-preserved-def with preserved-ev-r dst-ok
  ... | ev-r is-r refl = refl
  ...
  ¬elim-pres-dst : preserved-ev dst-ok ≢ elim-ev dst-ok
  ¬elim-pres-dst refl = po-irreflexive dst-wf refl (proj₁ (transform-pair dst-ok))

  ¬elim-pres-src : src-preserved-ev ≢ src-elim-ev
  ¬elim-pres-src q =
    let p≡e = ev[$⇒]eq (preserved∈ex dst-ok) (elim∈ex dst-ok) (≡-trans (≡-sym src-preserved-def) q)
  ...
  rfʳ-e⇒p : ∀ {x : Event LabelTCG} → rf src x src-elim-ev → rf src x src-preserved-ev
  ...
  rfʳ-e⇒p (rf-new dst-rf[xy] refl refl) =
    rf-dst (_ , _ , dst-rf[xy] , refl , ≡-trans src-preserved-def (ev[⇐]eq (preserved∈ex dst-ok) (rfʳ∈ex dst-wf dst-rf[xy])))

  frˡ-e⇒p : ∀ {y : Event LabelTCG} → fr src src-elim-ev y → fr src src-preserved-ev y
