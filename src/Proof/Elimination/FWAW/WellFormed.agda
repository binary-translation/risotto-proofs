...
src-co-init-first (co-newˡ dst-co[py] refl refl) y-init =
  ...
  in ⊥-elim (disjoint-init/rwₙₜ _ (pres-init , wₙₜ⇒rwₙₜ (pres₂-wₙᵣ dst-ok)))
...
src-co-init-first (co-ep refl refl) pres-init =
  ⊥-elim (disjoint-init/rwₙₜ _ (Init[$⇒] (pres₂∈ex dst-ok) pres-init , wₙₜ⇒rwₙₜ (pres₂-wₙᵣ dst-ok)))
...
src-co-trans (co-ep refl refl) (co-dst (_ , _ , co[py] , τ , refl))
  with ev[$⇒]eq (pres₂∈ex dst-ok) (coˡ∈ex dst-wf co[py]) τ
...
src-co-trans (co-ep refl refl) (co-newʳ co[pp] τ refl) =
  ⊥-elim (co-irreflexive dst-wf (≡-sym (ev[$⇒]eq (pres₂∈ex dst-ok) (coˡ∈ex dst-wf co[pp]) τ)) co[pp])
...
private
  ...
  pˢ-sloc-pᵗ : SameLoc src-pres₂-ev (pres₂-ev dst-ok)
  pˢ-sloc-pᵗ = same-loc src-pres₂-has-loc (pres₂-has-loc dst-ok)

  loc-eˢ⇒pᵗ : (loc : Location) → HasLoc loc src-elim-ev → HasLoc loc (pres₂-ev dst-ok)
  loc-eˢ⇒pᵗ _ = subst-sloc (trans-same-loc ep₂-sloc pˢ-sloc-pᵗ)

  coˡ-p⇒e : {y : Event LabelTCG} → co src src-pres₂-ev y → co src src-elim-ev y
  ...
  dst-pres₂-def : pres₂-ev dst-ok ≡ ev[⇒] pres₂∈src
  dst-pres₂-def = ev[⇒]eq (events[⇐] (pres₂∈ex dst-ok)) pres₂∈src (≡-sym src-pres₂-def)
  ...
  ... | no ¬y-elim
    with co-tri dst-wf (elim-loc dst-ok)
           (with-pred (pres₂-ev dst-ok) (pres₂-w dst-ok , pres₂-has-loc dst-ok , pres₂∈ex dst-ok))
  ...
  ... | tri<  co[py] p≢y ¬co[yp] =
    ...
    in
    tri<
      (coˡ-p⇒e (subst-rel (co src) (≡-sym src-pres₂-def) (≡-sym (ev[⇒⇐] y∈src)) (co[⇐] (pres₂∈ex dst-ok) (events[⇒] y∈src) co[py])))
      (λ{refl → ¬y-elim refl})
      (λ{co[ye] → ¬co[yp] (subst-rel (co dst) refl (ev[⇒]eq pres₂∈src (events[⇐] (pres₂∈ex dst-ok)) src-pres₂-def) (co[⇒] ¬y-elim ¬pres₂-elim y∈src pres₂∈src (coʳ-e⇒p co[ye])))})
      -- (co[⇒] ¬y-elim ? y∈src pres₂∈src (coʳ-e⇒p co[ye]))})
  ... | tri≈ ¬co[py] p≡y ¬co[yp] =
    let src-p≡y : src-pres₂-ev ≡ y
        src-p≡y = ≡-trans src-pres₂-def (ev[⇐$]eq (events[⇐] (pres₂∈ex dst-ok)) y∈src (with-pred-≡ p≡y))
    in
    tri<
...
slocˢᵗ : SameLoc src-pres₂-ev (ev[⇐] (pres₂∈ex dst-ok))
slocˢᵗ = same-loc src-pres₂-has-loc (loc[⇐] (pres₂∈ex dst-ok) (pres₂-has-loc dst-ok))
...
src-co-sloc = ⊆: lemma
  ...
  lemma e y (co-newˡ dst-co[py] refl refl) =
    ...
    in trans-same-loc (trans-same-loc ep₂-sloc slocˢᵗ) (sloc[⇐] (pres₂∈ex dst-ok) y∈dst py-sloc)
  lemma x e (co-newʳ dst-co[xp] refl refl) =
    let x∈dst = coˡ∈ex dst-wf dst-co[xp]
        xp-sloc = ⊆₂-apply (co-sloc dst-wf) dst-co[xp]
    in trans-same-loc (sloc[⇐] x∈dst (pres₂∈ex dst-ok) xp-sloc) (trans-same-loc (sym-same-loc slocˢᵗ) (sym-same-loc ep₂-sloc))
