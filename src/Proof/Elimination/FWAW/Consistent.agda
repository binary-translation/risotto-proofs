...
¬pres₁-init e-init = disjoint-f/init _ (pres₁-f dst-ok , e-init)

poˡ-e⇒p : {y : Event LabelTCG} → y ≢ src-pres₁-ev → y ≢ src-pres₂-ev → po src src-elim-ev y → po src src-pres₂-ev y
...
poʳ-e⇒p₂ : {x : Event LabelTCG} → po src x src-elim-ev → po src x src-pres₂-ev
poʳ-e⇒p₂ po[xe] = po-trans src-wf po[xe] po[ep₂]


plˡ-e⇒p : {y : Event LabelTCG} → y ≢ src-pres₂-ev → po-loc src src-elim-ev y → po-loc src src-pres₂-ev y
plˡ-e⇒p ¬y-pres₂ (po[ey] , ey-sloc@(same-loc _ y-has-loc)) =
  let ¬y-pres₁ = λ{refl → ¬f-loc (pres₁-f dst-ok) (_ , y-has-loc)}
  in poˡ-e⇒p ¬y-pres₁ ¬y-pres₂ po[ey] , trans-same-loc (sym-same-loc ep₂-sloc) ey-sloc

plʳ-e⇒p : {x : Event LabelTCG} → po-loc src x src-elim-ev → po-loc src x src-pres₂-ev
plʳ-e⇒p (po[xe] , xe-sloc) = poʳ-e⇒p₂ po[xe] , trans-same-loc xe-sloc ep₂-sloc


¬rfˡ-e : {y : Event LabelTCG} → ¬ rf src src-elim-ev y
¬rfˡ-e (_ , _ , rf[ey] , p , refl) with ev[$⇒]eq (elim∈ex dst-ok) (rfˡ∈ex dst-wf rf[ey]) p
... | refl = disjoint-w/skip _ (×₂-applyˡ (rf-w×r dst-wf) rf[ey] , elim-ev-skip dst-ok)


cohˡ-e⇒p : {y : Event LabelTCG} → y ≢ src-pres₂-ev → Coh src src-elim-ev y → Coh src src-pres₂-ev y
...
cohʳ-e⇒p : {x : Event LabelTCG} → Coh src x src-elim-ev → Coh src x src-pres₂-ev
...
poˡ-p⇒e : {y : Event LabelTCG} → po src src-pres₂-ev y → po src src-elim-ev y
poˡ-p⇒e = po-trans src-wf po[ep₂]

poʳ-p⇒e : {x : Event LabelTCG} → x ≢ src-elim-ev → x ≢ src-pres₁-ev → po src x src-pres₂-ev → po src x src-elim-ev
...
¬poʳ-e⇒p : {x : Event LabelTCG} → x ≢ src-elim-ev → x ≢ src-pres₁-ev → ¬ po src x src-elim-ev → ¬ po src x src-pres₂-ev
...
freʳ-e⇒p : {x : Event LabelTCG} → fre src x src-elim-ev → fre src x src-pres₂-ev
...
... | same-loc x-loc e-loc =
  let ¬x-elim = λ{refl → fr-irreflexive src-wf refl fr[xe]}
      ¬x-pres₁ = λ{refl → ¬f-loc (pres₁-f dst-ok) (_ , x-loc)}
  in frʳ-e⇒p fr[xe] , ¬poʳ-e⇒p ¬x-elim ¬x-pres₁ ¬po[xe]

coeˡ-e⇒p : {y : Event LabelTCG} → y ≢ src-pres₂-ev → coe src src-elim-ev y → coe src src-pres₂-ev y
...
coeʳ-e⇒p : {x : Event LabelTCG} → coe src x src-elim-ev → coe src x src-pres₂-ev
coeʳ-e⇒p (co[xe] , ¬po[xe]) =
  let ¬x-elim = λ{refl → co-irreflexive src-wf refl co[xe]}
      ¬x-pres₁ = λ{refl → disjoint-f/w _ (pres₁-f dst-ok , ×₂-applyˡ (co-w×w src-wf) co[xe])}
...
ordWWˡ-e⇒p₂ ¬y-pres₂ ((refl , e-w) ⨾[ _ ]⨾ po[ez] ⨾[ z ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ y ]⨾ (refl , y-w)) =
  ...
  lemma₁ po[p₁z] =
    let ¬z-pres₂ = λ{refl → disjoint-f/w _ (f₌⇒f z-f , src-pres₂-w)}
        ¬init-pres₁ = λ{p₁-init → disjoint-f/init _ (pres₁-f dst-ok , p₁-init)}
  ...
  ... | inj₁ y-wᵣ =
    ...
    in ord-ww ((refl , pres₂-w dst-ok) ⨾[ _ ]⨾ po[p₂x] ⨾[ _ ]⨾ (refl , x-ww) ⨾[ _ ]⨾ proj₁ pi[xy] ⨾[ _ ]⨾ (refl , y-w))
  ... | inj₂ y-wₐ =
    let po[p₂y] = elim-⊥⊎ (≢-sym ¬y-pres₂) id (unsplit-poˡ src-wf ¬pres₁-init po[p₁y] pi[p₁p₂])
    in ord-w ((refl , w⇒rw (pres₂-w dst-ok)) ⨾[ _ ]⨾ po[p₂y] ⨾[ _ ]⨾ (refl , y-wₐ))


ordˡ-e⇒p : {y : Event LabelTCG} → y ≢ src-pres₂-ev → OrdAlt src src-elim-ev y → OrdAlt src src-pres₂-ev y
...
ordˡ-e⇒p ¬y-pres₂ (ord-sc ((refl , e-rw) ⨾[ _ ]⨾ po[ez] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-rw))) =
  let ¬z-pres₁ = λ{refl → disjoint-f/mode (λ()) _ (z-f , pres₁-f₌ dst-ok)}
      ¬z-pres₂ = λ{refl → disjoint-f/w _ (f₌⇒f z-f , pres₂-w dst-ok)}
...
ordˡ-e⇒p ¬y-pres₂ (ord-rr rr[ey]) = ⊥-elim (disjoint-r/w _ (ord-predˡ src rr[ey] , src-elim-w))
ordˡ-e⇒p ¬y-pres₂ (ord-rw rw[ey]) = ⊥-elim (disjoint-r/w _ (ord-predˡ src rw[ey] , src-elim-w))
...
ordˡ-e⇒p ¬y-pres₂ (ord-wr wr[ey]) = ⊥-elim (src-no-wr _ (ord-fence∈src wr[ey] , ord-fence-F wr[ey]))
ordˡ-e⇒p ¬y-pres₂ (ord-wm wm[ey]) = ⊥-elim (src-no-wm _ (ord-fence∈src wm[ey] , ord-fence-F wm[ey]))
ordˡ-e⇒p ¬y-pres₂ (ord-mr mr[ey]) = ⊥-elim (src-no-mr _ (ord-fence∈src mr[ey] , ord-fence-F mr[ey]))
ordˡ-e⇒p ¬y-pres₂ (ord-mw mw[ey]) = ⊥-elim (src-no-mw _ (ord-fence∈src mw[ey] , ord-fence-F mw[ey]))
ordˡ-e⇒p ¬y-pres₂ (ord-mm mm[ey]) = ⊥-elim (src-no-mm _ (ord-fence∈src mm[ey] , ord-fence-F mm[ey]))


ordfʳ-e⇒p : {P₁ P₂ : Pred (Event LabelTCG) ℓzero} {f : F-mode}
  → {x : Event LabelTCG}
...
ordʳ-e⇒p : {x : Event LabelTCG} → OrdAlt src x src-elim-ev → OrdAlt src x src-pres₂-ev
...
-- impossible cases
ordʳ-e⇒p (ord-rr rr[xe]) = ⊥-elim (disjoint-r/w _ (ord-predʳ src rr[xe] , src-elim-w))
ordʳ-e⇒p (ord-wr wr[xe]) = ⊥-elim (disjoint-r/w _ (ord-predʳ src wr[xe] , src-elim-w))
ordʳ-e⇒p (ord-mr mr[xe]) = ⊥-elim (disjoint-r/w _ (ord-predʳ src mr[xe] , src-elim-w))
...
ordʳ-e⇒p (ord-rw rw[xe]) = ⊥-elim (src-no-rw _ (ord-fence∈src rw[xe] , ord-fence-F rw[xe]))
ordʳ-e⇒p (ord-wm wm[xe]) = ⊥-elim (src-no-wm _ (ord-fence∈src wm[xe] , ord-fence-F wm[xe]))
ordʳ-e⇒p (ord-mw mw[xe]) = ⊥-elim (src-no-mw _ (ord-fence∈src mw[xe] , ord-fence-F mw[xe]))
ordʳ-e⇒p (ord-mm mm[xe]) = ⊥-elim (src-no-mm _ (ord-fence∈src mm[xe] , ord-fence-F mm[xe]))


ghbiˡ-e⇒p : {y : Event LabelTCG} → y ≢ src-pres₂-ev → GhbiAlt src src-elim-ev y → GhbiAlt src src-pres₂-ev y
...
ghbiʳ-e⇒p : {x : Event LabelTCG} → GhbiAlt src x src-elim-ev → GhbiAlt src x src-pres₂-ev
