...
open import Arch.TCG using (EvF₌; F-mode)
-- Local imports: Proof Components
import Proof.Framework LabelTCG dst dst-wf as Ψ
-- Local imports: Other
open import Helpers as H
...
module Types
  ...
  CPred[$⇒] : Pred (Event LabelTCG) ℓzero → Pred (Event LabelTCG) ℓzero → Set
  CPred[$⇒] Pˢ Pᵗ = ∀ {x}
    → x ≢ elim-ev
    → (x∈dst : x ∈ events dst)
    → Pˢ (ev[⇐] x∈dst)
  ...
  CRel[$⇒] : Rel (Event LabelTCG) ℓzero → Rel (Event LabelTCG) ℓzero → Set
  CRel[$⇒] Rˢ Rᵗ = ∀ {x y}
    → x ≢ elim-ev → y ≢ elim-ev
  ...
  CPred[$⇒]² : Pred (Event LabelTCG) ℓzero → Set
  CPred[$⇒]² P = CPred[$⇒] P P
...
module Definitions (δ : EliminationFramework) where
  ...
  CRel[⇒]² : Rel (Event LabelTCG) ℓzero → Set
  CRel[⇒]² R = CRel[⇒] R R
  ...
  [$⇒]→₁[⇒]ᶜ :
      {Pˢ : Pred (Event LabelTCG) ℓzero}
    → {Pᵗ : Pred (Event LabelTCG) ℓzero}
    → CPred[$⇒] Pˢ Pᵗ
      ---------------
    → CPred[⇒] Pˢ Pᵗ
  [$⇒]→₁[⇒]ᶜ {Pˢ} P[$⇒] ¬x-elim x∈src =
  ...
  [$⇒]→₂[⇒]ᶜ :
      {Rˢ : Rel (Event LabelTCG) ℓzero}
    → {Rᵗ : Rel (Event LabelTCG) ℓzero}
    → CRel[$⇒] Rˢ Rᵗ
      ---------------
    → CRel[⇒] Rˢ Rᵗ
  ...
  -- # R
  ord[⇒] ¬x-elim ¬y-elim x∈src y∈src (ord-rr ((refl , x-r) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-r))) =
    let z∈src = poʳ∈src po[xz]
    in ord-rr ((refl , R[⇒] ¬x-elim x∈src x-r) ⨾[ _ ]⨾ po[⇒] x∈src z∈src po[xz] ⨾[ _ ]⨾ (refl , F₌[⇒] z∈src z-f) ⨾[ _ ]⨾ po[⇒] z∈src y∈src po[zy] ⨾[ _ ]⨾ (refl , R[⇒] ¬y-elim y∈src y-r))
  ord[⇒] ¬x-elim ¬y-elim x∈src y∈src (ord-rw ((refl , x-r) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-w))) =
    let z∈src = poʳ∈src po[xz]
    in ord-rw ((refl , R[⇒] ¬x-elim x∈src x-r) ⨾[ _ ]⨾ po[⇒] x∈src z∈src po[xz] ⨾[ _ ]⨾ (refl , F₌[⇒] z∈src z-f) ⨾[ _ ]⨾ po[⇒] z∈src y∈src po[zy] ⨾[ _ ]⨾ (refl , W[⇒] ¬y-elim y∈src y-w))
  ...
  -- # W
  ord[⇒] ¬x-elim ¬y-elim x∈src y∈src (ord-wr ((refl , x-w) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-r))) =
    let z∈src = poʳ∈src po[xz]
    in ord-wr ((refl , W[⇒] ¬x-elim x∈src x-w) ⨾[ _ ]⨾ po[⇒] x∈src z∈src po[xz] ⨾[ _ ]⨾ (refl , F₌[⇒] z∈src z-f) ⨾[ _ ]⨾ po[⇒] z∈src y∈src po[zy] ⨾[ _ ]⨾ (refl , R[⇒] ¬y-elim y∈src y-r))
  ...
  ord[⇒] ¬x-elim ¬y-elim x∈src y∈src (ord-wm ((refl , x-w) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-m))) =
    let z∈src = poʳ∈src po[xz]
    in ord-wm ((refl , W[⇒] ¬x-elim x∈src x-w) ⨾[ _ ]⨾ po[⇒] x∈src z∈src po[xz] ⨾[ _ ]⨾ (refl , F₌[⇒] z∈src z-f) ⨾[ _ ]⨾ po[⇒] z∈src y∈src po[zy] ⨾[ _ ]⨾ (refl , RW[⇒] ¬y-elim y∈src y-m))
  -- # R/W
  ord[⇒] ¬x-elim ¬y-elim x∈src y∈src (ord-mr ((refl , x-m) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-r))) =
    let z∈src = poʳ∈src po[xz]
    in ord-mr ((refl , RW[⇒] ¬x-elim x∈src x-m) ⨾[ _ ]⨾ po[⇒] x∈src z∈src po[xz] ⨾[ _ ]⨾ (refl , F₌[⇒] z∈src z-f) ⨾[ _ ]⨾ po[⇒] z∈src y∈src po[zy] ⨾[ _ ]⨾ (refl , R[⇒] ¬y-elim y∈src y-r))
  ord[⇒] ¬x-elim ¬y-elim x∈src y∈src (ord-mw ((refl , x-m) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-w))) =
    let z∈src = poʳ∈src po[xz]
    in ord-mw ((refl , RW[⇒] ¬x-elim x∈src x-m) ⨾[ _ ]⨾ po[⇒] x∈src z∈src po[xz] ⨾[ _ ]⨾ (refl , F₌[⇒] z∈src z-f) ⨾[ _ ]⨾ po[⇒] z∈src y∈src po[zy] ⨾[ _ ]⨾ (refl , W[⇒] ¬y-elim y∈src y-w))
  ord[⇒] ¬x-elim ¬y-elim x∈src y∈src (ord-mm ((refl , x-m) ⨾[ _ ]⨾ po[xz] ⨾[ _ ]⨾ (refl , z-f) ⨾[ _ ]⨾ po[zy] ⨾[ _ ]⨾ (refl , y-m))) =
    let z∈src = poʳ∈src po[xz]
    in ord-mm ((refl , RW[⇒] ¬x-elim x∈src x-m) ⨾[ _ ]⨾ po[⇒] x∈src z∈src po[xz] ⨾[ _ ]⨾ (refl , F₌[⇒] z∈src z-f) ⨾[ _ ]⨾ po[⇒] z∈src y∈src po[zy] ⨾[ _ ]⨾ (refl , RW[⇒] ¬y-elim y∈src y-m))
  -- # SC
